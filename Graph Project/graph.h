...
public:
  ...
  bool connect(const string &startingPoint, const string &endingPoint, int weight = 0);

  // Remove edge from graph
  // @return true if edge successfully deleted
  bool disconnect(const string &startingPoint, const string &endingPoint);
  ...
  int mstPrim(const string &startLabel,
              void visit(const string &startingPoint, const string &endingPoint,
                         int weight)) const;
...
private:
  // creating map for graph
  map<string, Vertex *> graph;

  // if it is a directionalEdge
